...
def extract_dist_cst_from_pdb_use_allatm(pdb_in,ptm_resname,bsite_res=''):
    #
    #use all_res (only chainA that is not a peptide)
    if bsite_res == '':
        bsite_res_s = []
    ...
    with open(pdb_in) as fp:
        for line in fp:
            if line.startswith('ATOM'):
                ...
                if atm.chainID == 'A' and atm.resNo in bsite_res_s and atm.atmName == 'CA':
                    bsite_CA_R[atm.resNo] = atm.coord
            elif line.startswith('HETATM'):
                atm = Atom(line.strip())
                if atm.resName == ptm_resname:
                    het_resno = last_prot_resno + 1
                    lig_tr_atms.append(atm.atmName)
