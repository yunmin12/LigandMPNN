...
def main():
    ...
    ap.add_argument("--mode", choices=["tar","off"], required=True)
    ap.add_argument("--target-chain", default="Z")  # ligand chain if you also want to treat HETATM
    ap.add_argument("--update-ligand", type=int, default=0)  # 1 to also update ligand atoms (chain Z)
    args = ap.parse_args()

    subdirs = sorted([d for d in glob.glob(os.path.join(args.base_dir, "*"))])
    for sub in subdirs:
        cleaned_dir = os.path.join(sub, f"cleaned")
        split_dir   = os.path.join(sub, f"split")
        recover_dir = os.path.join(sub, f"recover")
